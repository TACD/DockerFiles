...
if [ ! -e model.xml ]; then                                                
	...
	# Move model.xml to temp directory
	cp -f model.xml ${TEMP_DIR}
	...
	for file in *.bin; do
		...
		if [ -f ${FILE_CSV} ]; then  
			...
			sed -i "s/\(${STR_MATCH}\)[0-9]\+/\1${CSV_LINES}/" ${TEMP_DIR}/model.xml    
		else
			# Report if no matching CSV found
			echo "(!) ${FILE_CSV} not found"                                
		fi
	done
	# Return model.xml to current directory
	cp -f ${TEMP_DIR}/model.xml .
fi
